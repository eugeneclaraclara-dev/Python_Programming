...
# [x]: x 자체를 원소 하나로 해서 넣음
print([zip(subjects, scores)])  # zip 객체 통으로 들어감
...
print([x**2 for x in range(1, 11)])  # WOW
...
result = ["짝수" if x % 2 == 0 else "홀수" for x in range(1, 11)]
print(result)
# 각 이름의 길이로 이루어진 리스트 만들기
names = ["pororo", "crong", "poby", "eddy"]
# [6, 5, 4, 4]
nll = [len(name) for name in names]
print(nll)

# 길이가 5 이상인 이름만 뽑기
nl = [name for name in names if len(name) >= 5]
print(nl)
# 중첩 for문도 가능
# x = 0 1 2
# y = 0 1 2
